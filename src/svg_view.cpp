...
void SvgView::render(const Maze& maze) const
{
    ...
    // Add right and bottom bounds:
    svg->append(new Line{
        {"x1", std::to_string(maze.get_width() * cell_width)},
        {"y1", std::to_string(0)},
        {"x2", std::to_string(maze.get_width() * cell_width)},
        {"y2", std::to_string(maze.get_height() * cell_height)}
    });
    svg->append(new Line{
        {"x1", std::to_string(0)},
        {"y1", std::to_string(maze.get_height() * cell_height)},
        {"x2", std::to_string(maze.get_width() * cell_width)},
        {"y2", std::to_string(maze.get_height() * cell_height)}
    });
    out << html;
